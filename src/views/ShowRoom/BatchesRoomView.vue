<template>
  <div class="flex w-full">
    ...
    <div class="w-full py-10 px-4">
      ...
      <div
        class="p-5 grid gird-cols-1 text-center md:grid-cols-2 xl:grid-cols-5 lg:grid-cols-4 gap-6 mb-5"
        v-else
      >
        <template v-if="itemsBatches?.length > 0">
          <div
            v-for="i in itemsBatches"
            :key="i.batch_id"
          ...
          >
            ...
            <div class="mt-3">
              ...
              <p class="text-gray-600 text-sm">
                Material:
                <span class="font-medium">{{ i.material_nicknames }}</span>
              </p>
              <p class="text-gray-600 text-sm">
                Category: <span class="font-medium">{{ i.category_name }}</span>
              </p>
              <p class="text-gray-600 text-sm">
                Shape: <span class="font-medium">{{ i.shape_name }}</span>
              </p>
              <p class="text-gray-600 text-sm">
                Size:
                <span class="font-medium"
                  >{{ i.length }} × {{ i.width }} mm</span
                >
              </p>
              <p class="text-gray-600 text-sm">
                Avg Price:
                <span class="font-medium">{{ i.avg_price }} EGP</span>
              </p>

              <p
                v-if="i.item_batch_location?.length"
                class="text-gray-600 text-sm"
              >
                Stock Quantity:
                <span class="font-medium">{{
                  i.item_batch_location[0].quantity
                }}</span>
              </p>
              <router-link
                :to="{ name: 'History', params: { id: i.id } }"
...
const getItemAndFilters = async () => {
  try {
    filters.value.materialNicknames = materialNicknames.value;
    loading.value = true;
    const response = await show_room_api.getBatchesWithDropDown(
      filters.value,
      +route.params.id
    );
    itemsBatches.value = response.batches;
