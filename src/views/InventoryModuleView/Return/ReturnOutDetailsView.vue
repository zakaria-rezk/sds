  ...
  <section class="mx-8">
    <div>
      ...
      <TableLayoutComponent
        ...
        :all-fields="opHeader"
        :pageLable="$t('rin_form.BreadCrump')"
        :slotTDcolName="slotTDcolName"
        @getUpdatedData="getRout()"
      >
...
const breadCrumps = [
  {
    text: t("rin_form.rin_list"),
    route: "Return-Out",
    params: { operation_id: route.params.operation_id },
  },
  { text: t("rin_form.rin_details"), route: "" },
];
...
const getRout = async () => {
  loading.value = true;
  try {
    const response = await return_out_api.getSpecificRinFormItem(
    ...
    rows.value = response.form_details.map((item: any) => ({
      ...
      item_name: item.item_batch_location_full_details?.item_name,
      material_name: item.item_batch_location_full_details?.material_name || "",
      category_name: item.item_batch_location_full_details?.category_name || "",
      ...
      date:
        ...
        (item.item_batch_location_full_details?.production_date?.slice(0, 10) ||
          "") +
        "<br>" +
        (t("operation_details.rows.experation_date") +
          (item.item_batch_location_full_details?.exp_date?.slice(0, 10) ||
            " ")),
      finshing_level:
        item.item_batch_location_full_details?.finishing_level_name || "",
      additional_data: { status: item.status, detail_id: item.id },
    }));
    opHeader.value = [
      {
...
//   await return_out_api.deleteItem(+row_id);
//   getRout();
// };
...
const updateItemDetails = async (
  ...
  await return_out_api.updateItemDetails(params, +row_id);
  getRout();
};
//FORM
//FORM IMPORTS
...
const submitForm = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      item_batch_location: row_id.value,
    };
    await return_out_api.updateItemDetails(payload, detail_id.value);
    getRout();
