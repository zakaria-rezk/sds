...
import return_out_api from "@/services/inventory/return_in_api";
...
const filter = async (filteredData: any = null) => {
  filteredData.r_out_form_id = +route.params.form_id;
     console.log("sss")
  try {
    loading.value = true;
    const response = await return_out_api.getOutBatches(filteredData);
    rows.value = response.map((item: any) => ({
      ...
      date:
        ...
        (item.item_batch_location_full_details.item_batch_full_details?.production_date?.slice(
          0,
          10
        ) || " ") +
        "<br>" +
        (t("operation_details.rows.experation_date") +
          (item.item_batch_location_full_details.item_batch_full_details?.exp_date?.slice(
            0,
            10
          ) || " ")),
...
const getDropDownFilterData = async () => {
  try {
    filterDropDownData.value = await return_out_api.getDropDownFilterData();
  } catch (error) {
  } finally {
    loading.value = false;
  }
...
const addItemsToForm = async () => {
  ...
  try {
    ...
    await return_out_api.addItemsToRinForm(payload);
